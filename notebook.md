...
- 类和对象的概念
    - 类：同的属性用来描述具有相和方法的对象的集合。
    抽象名词，代表一个集合，共性的事物。
    - 对象：具象的事物，单个个体。对象是类的实例。
    - 类跟对象的关系
        - 一个具象代表一类事物的某一个个体
        - 一个是抽象，代表的是一大类事物。
    - 类中的内容，应该具有两个内容
        - 表明事物的特征，叫做属性（变量）
        - 表明事物的动作或功能，叫做成员方法（类中定义的函数）
#2.类的基本实现。
   - 类的命名
        - 遵守变量命名的规范。
        - 大驼峰
   ...
   - 实例化类
    变量 = 类名（） #实例化了一个对象
   - 类变量
        - 类变量在整个实例化的对象中是公用的。类变量定义在类中且在函数体外。
        类变量通常不作为实例变量使用。
   - 数据成员
   ...
   - 实例变量
        - 在类的声明中，属性是用变量来表示的，这种变量就成为实例变量，是在类声明的内部但是在类的其他成员方法之外声明的。
   - 访问对象成员
        - 使用点操作符可以访问对象的属性
            obj.成员属性名称
            obj.成员方法
...
- 单继承与多继承的优缺点
    ...
    - 多继承：
        ...
        - 继承关系很混乱
- 构造函数
    - 在对象进行实例化的时候，系统自动调用的一个函数叫构造函数，通常此函数用来对
    实例化对象进行
...
- 优点
    - 使用Mixin可以在部队类进行进行任何修改的情况下，扩充功能。
    - 可以方便的组织和维护不同功能组件的划分。
    - 可以根据需要任意调整功能类的组合。
